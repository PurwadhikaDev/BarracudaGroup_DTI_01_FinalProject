...
def plot_roc_models(model_dict, anotate=False):
    """
    This function is created in order to plot the roc curve for different models
    Parameters: model_dict: dictionary, containing the fpr, treshold, and auc value
                            of different models
                Annotate : bool, optional ; to show the annotations of treshold

    Returns: None, it just plots the roc curve
    """
    ...
    for idx, (model_name, model_data) in enumerate(model_dict.items()):
        tpr = model_data["tpr"]
        thresholds = model_data["threshold"]
        roc_auc = model_data["auc"]

        plt.plot(fpr, tpr, lw=2, label=f"{model_name} (AUC = {roc_auc:.2f})")

        if anotate:
            # Define different offsets for different models
            offsets = {
                0: (10, 10),
                1: (-10, 10),
                2: (10, -10),
            ...
            # Annotate the thresholds at every 0.1 FPR only for the first model
            for i in range(len(fpr)):
                if np.isclose(fpr[i], np.arange(0.1, 1.1, 0.1), atol=0.001).any():
                    offset = offsets[
                        idx % len(offsets)
                    ]  # Use different offset for each model
                    plt.annotate(
                        f"{thresholds[i]:.2f}",
                        (fpr[i], tpr[i]),
    ...
    plt.title("Receiver Operating Characteristic (ROC) Curve Across Models")
...
def plot_roc_standard(tpr, thresholds, roc_auc):
    ...
    for i in range(len(fpr)):
        if np.isclose(
            fpr[i], np.arange(0.1, 1.1, 0.1), atol=0.001
        ).any():  # Check if mean_fpr is close to 0.1, 0.2, ..., 1.0
            plt.annotate(
                f"{thresholds[i]:.2f}",
                (fpr[i], tpr[i]),
                textcoords="offset points",
